...
import net.assentindia.edi.processor.reader.ContentHandlerBase64Encoder;
import net.assentindia.edi.processor.reader.EDIAttributes;
import net.assentindia.edi.processor.reader.EDIStandard;
import net.assentindia.edi.processor.reader.FixedLength;
import net.assentindia.edi.processor.reader.ReplyGenerator;
import net.assentindia.edi.processor.reader.StandardReader;
import net.assentindia.edi.processor.reader.Token;
import net.assentindia.edi.processor.reader.exception.EDISyntaxException;
import net.assentindia.edi.processor.reader.exception.ISA16SubElementDelimiterException;
import net.assentindia.edi.processor.reader.exception.ISAFixedLengthException;
import net.assentindia.edi.processor.reader.exception.MissingMandatoryElementException;
...
import net.assentindia.edi.processor.reader.queue.QueuedContentHandler;

import java.io.IOException;
import java.lang.invoke.MethodHandles;

import static net.assentindia.edi.processor.reader.Token.TokenType.SEGMENT_END;
import static net.assentindia.edi.processor.reader.Token.TokenType.SEGMENT_START;
...
public class AnsiReader extends StandardReader {
    ...
    protected Token parseDocument(Token token) throws SAXException,
        ...
        logger.info("Parsing {} Plugin Controller", pluginController.getDocumentName());
        ...
        if (!hitSegmentEnd) {
            ...
            logger.info("Token {} st03Token", st03Token.getValue());
    ...
    public void preview() throws EDISyntaxException, IOException {
        ...
        // ...^ (offset 3)
        char c = buf[3];
        logger.info("Searching and Setting delimeter {}",c);
        setDelimiter(c);
        ...
        if (isAcceptable(c))
        {
        	logger.info("Searching and Setting sub delimeter {}",c);
        	setSubDelimiter(c);
        }
        else {
        ...
        if (isAcceptable(c))
        {
        	logger.info("Searching and Setting terminator {}",c);
            setTerminator(c);
        }
        else {
        ...
        String terminatorSuffix =findTerminatorSuffix(buf, indexOf16thFieldSeparator + 3, 128);
        logger.info("Searching and Setting Terminator Suffix {}",terminatorSuffix);
        setTerminatorSuffix(terminatorSuffix);
